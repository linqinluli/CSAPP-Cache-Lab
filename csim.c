...
#include<string.h> 
int s, E, b, S;
int number_hits=0, number_miss=0, number_eviction=0;
...
typedef struct 
{
	int valid_bit, tag, stamp;//data structure of the cache
}cache_line;
...
{
	int opt=0, temp;//The getopt return value
	...
	for(int i = 0; i < S; i++)
		cache[i] = (cache_line*)malloc(sizeof(cache_line) * E);//malloc each row of cache
	for(int i = 0; i < S; i++)
		for(int j = 0; j < E; j++)
		{
